...
// Option configures a [Handler] via [NewHandler].
...
func (c config) TracerProvider(ctx context.Context) (trace.TracerProvider, error) {
	exp := c.exporter
	if exp == nil {
		var err error
...
func (c config) resource() *resource.Resource {
	...
	if ok {
		...
		for _, mod := range bi.Deps {
			if mod.Path == "go.opentelemetry.io/auto" {
				attrs = append(
					attrs,
					semconv.TelemetryDistroVersionKey.String(mod.Version),
				)
				break
			}
		}
